...
- PR #329 Demolition Pricing Safeguards is current in this PR and ready for merge: runtime demolition quotes now use narrow `app/quote_engine.py` floor protection for controlled demolition, normal demolition, unknown scope, hazardous or regulated unknown-scope wording, access-risk wording, structure teardown wording, heavy-material wording, and heavy-plus-access combinations. These safeguards are still internal pricing-engine behavior only; GPT must describe them as repository behavior, not create alternate pricing logic or override owner review.
