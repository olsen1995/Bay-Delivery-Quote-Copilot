...
- Live production Render admin contains at least two manual completed-job calibration entries in the admin log: an old shed removal and haul-away calibration on 18 May (type: demolition, $1,200 CAD, crew 2, duration 3 hours, labour-hours 6.00, profitable, disassembly required), and a backyard cleanup with tarp fence teardown calibration on 18 May (type: haul_away, $600 CAD, crew 3, duration 3 hours, labour-hours 9.00, profitable, disassembly required).
- Calibration lesson: these entries are historical owner-review evidence that shed removal and cleanup jobs involving teardown/disassembly can require premium demolition/haul-away treatment rather than basic dump-run assumptions. They should inform future owner review and pricing calibration analysis only; they do not set standalone quote ranges, do not override `app/quote_engine.py`, and do not create a second pricing path.
- Important audit note: local SQLite files under `app/data` may be empty, test-generated, or unrelated to production and must not be treated as production calibration truth.
- Important pricing authority note: manual calibration entries are evidence for future owner review and pricing calibration; they do not override `app/quote_engine.py` and do not create a second pricing path.
- Dump/disposal calibration is now represented in `app/quote_engine.py`: dump/disposal haul-away jobs can use a North Bay internal route default of approximately 50 km / 48 minutes round trip only when no better route or time data is supplied. This does not create a broad route-distance system and does not apply to ordinary small moves, item deliveries, demolition jobs, scrap pickup, or non-disposal work.
- Mattress and box spring customer charge calibration is now $60 each, while underlying landfill-cost assumptions remain internal and are not customer-facing line items.
- A completed high-risk appliance/moving job from 2026-06-13 is documented as internal calibration evidence: $1,200 CAD charged, 4 workers, approximately 6-7 hours each, 24-28 labour-hours, and high item/property care factors. Similar 4-worker, 6+ hour high-care appliance/moving jobs should trigger internal owner review and may calibrate roughly around $1,100-$1,400 depending on crew, time, access, travel, item risk, and property risk. This is not a hardcoded quote rule, does not expose the exact address, and does not override `app/quote_engine.py`.
...
- PR #330 "create GPT current state refresh after demolition safeguards" was post-merge verified on main at commit `37ecc3e` and version `0.12.0`; it is a historical GPT grounding baseline, not the current main baseline.
- PR #345 "create structured access pricing guardrails" is merged and verified. It added structured non-demolition access pricing guardrails in `app/quote_engine.py`: `floor_count`, `stairs_count`, and `basement_or_inside_removal` can upgrade normal access when structured intake indicates harder access, explicit difficult/extreme access is not downgraded, and demolition safeguards remain separate rather than double-counted. Advisory metadata remains internal and does not control price. Full pytest passed and production live-safe smoke passed against `edf5d99`.
- PR #346 "update codex prompt sources for skills and agents layout" was the prior docs/prompt-source baseline after the Codex app layout guidance refresh.
- PR #355 "fix scrap inside removal pricing" is a historical pricing baseline. It fixed inside scrap removal pricing: curbside scrap remains `$60` cash / `$67.80` EMT, and inside scrap is now `$90` cash / `$101.70` EMT. README, GPT business rules, grounding pack, and disclaimer wording were aligned in PR #355.
- PR #357 "clarify GPT Action YAML guidance" is merged. It clarified GPT Action YAML guidance only and did not change runtime, pricing, auth, config, Render, workflows, dependencies, `VERSION`, static/admin behavior, SQLite behavior, or grounding-pack behavior. GPT Builder Actions were manually tested successfully after the auth fix: `getGptQuote` works and `createGptAdminNote` works.
- PR #358 "align mattress box spring config pricing" is merged. Mattress/boxspring config is now aligned to `$60` each. No runtime pricing behavior changed.
- PR #360 "create admin import restore safety guards" is merged and verified. It added admin database import and Google Drive restore confirmation and dry-run safeguards.
- PR #361 "fix startup quote request duplicate handling" is merged and is the historical verified runtime baseline at `bf56cee fix startup quote request duplicate handling (#361)`. That baseline was verified at version `0.12.0`; post-merge verification passed version parity, GPT grounding parity, compileall, storage duplicate tests (10 passed), adjacent focused tests (33 passed), full pytest (1367 passed, 1 existing Starlette/httpx warning), protected no-go diff, and production live-safe smoke. It made startup `quote_request` duplicate handling non-destructive, preserved duplicate `quote_requests` during backup import/restore, made ambiguous duplicate `quote_id` lookups fail closed, and guarded future duplicate `quote_id` writes when the unique index is skipped.
- PR #362 "update version after storage safety fixes" is the current `0.13.0` release-marker metadata update, with initial version-bump commit `354b6e8 update version after storage safety fixes`. It documents PR #360 and PR #361 as verified safety changes and does not change runtime, pricing, storage behavior, workflows, Render config, dependencies, static assets, or GPT action schema wording.
...
- Demolition pricing safeguards are live through PR #329, and structured non-demolition access pricing guardrails are current through PR #345. Future pricing calibration for other service categories and any later heavy-category refinements remains separate future work and must not be treated as complete.
...
- Keep high-risk appliance/move calibration advisory-only unless a future owner-approved pricing PR changes `app/quote_engine.py`; do not hardcode internal addresses into pricing logic.
...
- Keep future pricing calibration beyond the live PR #329 demolition safeguards and PR #345 structured-access guardrails deferred to later category-specific PRs after evidence review; do not imply all pricing work is complete.
...
- No pricing influence, no schema migration, and no mobile-admin changes came from PR #291 lead source + repeat customer tracking. `app/quote_engine.py` pricing changes after that work include PR #329 demolition safeguards and PR #345 structured non-demolition access pricing guardrails; it remains the only pricing authority.
