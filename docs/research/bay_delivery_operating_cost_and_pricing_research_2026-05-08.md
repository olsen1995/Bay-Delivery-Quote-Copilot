...
For dump-run economics, local disposal pricing matters enough that it should almost never be buried inside a vague hourly quote. At the Merrick landfill, current City pricing includes **$30** for a truck or trailer up to 1,000 kg and then additional weight-based charges, with separate extras for items like **mattresses/box springs** and **propane tanks**. That makes “all-in disposal included” a margin trap on heavy mixed loads. Bay Delivery should treat disposal as **actual dump cost plus a handling/admin margin**, or use a tightly controlled load-based matrix.
...
| Merrick landfill fee structure | Current North Bay fees include **$30** for truck/trailer loads up to **1,000 kg**, then extra weight-based charges, plus line-item fees for **mattresses/box springs ($12 each)** and **propane tanks (from $10 if punctured)**. | Disposal must be a separate priced component. Heavy junk loads can wipe out margin if Bay quotes them like simple labour jobs. |
...
| “Dump fees should be passed through separately or tightly controlled.” | **True** | **High** | Merrick’s fee structure is weight-based and includes item-specific charges, which creates margin volatility on mixed loads. |
...
| Disposal | **Actual landfill/disposal cost + handling margin**, or strict load/weight tiers | Merrick fees are weight-based and create margin blowouts on heavy loads. |
...
- **Landfill / disposal:** actual cost plus handling margin, with special item surcharges for mattresses, propane tanks, and other fee-trigger items
