...
async def test_quote_estimate_breakdown_and_decline_path(page: Page, live_server: str) -> None:
    await page.goto(f"{live_server}/quote", wait_until="networkidle")
    await expect(page.locator("#quoteForm")).to_be_visible()
    await expect(page.locator("#quoteForm")).to_contain_text("Answer what you can. Not sure is okay.")
    await expect(page.locator("#quoteForm")).to_contain_text("Use full kitchen-size bags as a rough count.")
    await expect(page.locator("#quoteForm")).to_contain_text("Heavy items help Bay Delivery bring the right setup.")
    await expect(page.locator("#quoteForm")).to_contain_text("Choose the closest option and add a quick note if you are not sure.")
    await expect(page.locator("#quoteForm")).to_contain_text("Photos help us confirm scope faster.")
    ...
    await expect(page.locator("#resultBox")).to_contain_text("Photos are optional after your booking request if they help Bay Delivery confirm scope.")
