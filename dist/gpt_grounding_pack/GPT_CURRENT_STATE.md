...
- PR #355 "fix scrap inside removal pricing" is merged, live, and verified as the current main baseline. Current local/GitHub `main` after PR #355 is `6bca6dadfca3b12aa9c81fb9ebd5a5e7cf5a5f06`, version remains `0.12.0`, Austin verified live Render `/health` at commit prefix `6bca6dadfca3` with `drive_configured=true`, version parity passed, GPT grounding parity passed with 9 files matching manifest and sources, `tests/test_pricing_invariants.py` passed with 586 tests, and `tests/test_quote_boundary_validation.py` passed with 12 tests. PR #355 fixed inside scrap removal pricing: curbside scrap remains `$60` cash / `$67.80` EMT, and inside scrap is now `$90` cash / `$101.70` EMT. README, GPT business rules, grounding pack, and disclaimer wording were aligned in PR #355.
