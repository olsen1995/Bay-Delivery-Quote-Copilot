...
description: Use when reviewing or changing Bay Delivery SQLite persistence, including writes, constraints, save/update/upsert paths, review-required records, reconstruction, listings, queues, imports, restores, backups, migrations, duplicate handling, startup integrity, or compatibility behavior.
---
...
## Review-Required Operational Records

Trigger this section when review-required or otherwise non-authoritative quote records are stored, reconstructed, listed, exported, imported, restored, linked, token-rotated, queued, or exposed through admin/customer workflows.

These are operational review records, not customer-acceptable quotes. Across every applicable path, prove:
...
5. A real restore rejects a review-required quote linked to a `quote_request`, but preview reports success.
6. A review-required quote linked to a job regains an approval or booking workflow.
