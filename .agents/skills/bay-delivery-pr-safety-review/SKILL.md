...
Do not introduce React, Supabase, Stripe, complex frontend build systems, or unrelated architecture unless Austin explicitly asks and approves.
...
- Prefer narrow PRs.
- Prefer full-file replacements when directly providing code.
...
If changing requirements.txt or requirements.lock.txt, use Linux/Docker/WSL for lock generation.

Do not regenerate lockfiles with native Windows pip-compile because Windows can add platform-specific entries such as colorama that GitHub Ubuntu CI removes.
...
Never mutate production data unless Austin explicitly approves.
